/*
Message Decoding  Share on LinkedIn
...
int main(int argc, char *argv[]) {
	...
	// hard coding strings, should be able to change    
    table.push_back("0");
    table.push_back("00");
    table.push_back("01");
    table.push_back("10");
    ...
    while (getline(ifs, line)) {
    	int pos = min(line.find('0'), line.find('1'));
    	...
    	while (1) {
    		...
    		string cur = msg.substr(i, len);
    		while (dict.find(cur) != dict.end()) {
    			cout << dict[cur];
    			i += len;
    			cur = msg.substr(i, len);
    		}
    		i += len;
    	}
    }
